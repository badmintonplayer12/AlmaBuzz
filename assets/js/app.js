...
if (!elements.actionButton || !elements.statusLine) {
  console.warn("AlmaBuzz UI elements missing; check markup.");
}
...
function buildPreviewSrc(file) {
  ...
  const base = manifestRef.basePath ?? "assets/sounds/almabuzz";
...
function renderLibraryList() {
  ...
  if (elements.libraryList.childNodes.length) {
    if (shouldShowUpdateBanner) {
      ...
      installBtn.textContent = "Installer AlmaBuzz";
...
async function bootstrap() {
  console.info("AlmaBuzz bootstrap (Phase 2)");
  bindUi();
  ...
  console.info("AlmaBuzz state snapshot", {
    manifest,
    manifestError: error,
...
const run = () => {
  bootstrap().catch((err) => {
    console.error("AlmaBuzz bootstrap failed", err);
  });
};
...
if (typeof window !== "undefined") {
  ...
  console.info(
    "AlmaBuzz debug helpers available via window.almaBuzzDebug",
    "\nTry: almaBuzzDebug.getState()",
    "\nOr: almaBuzzDebug.playlist.snapshot()",
  );
}
...
async function handleShare() {
  const shareData = {
    title: "AlmaBuzz",
    text: "Installer AlmaBuzz og spill neste bingo-lyd med ett trykk!",
