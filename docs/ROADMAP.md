# AlmaBuzz Implementation Roadmap

## 1. Project Scaffolding
- Init basic structure: `index.html`, `assets/css/style.css`, `assets/js/{app.js,audio-engine.js,playlist.js,storage.js,constants.js}`, `assets/sounds/almabuzz/manifest.json`.
...
- **Forberedelser**
  ...
  - Debug-verktøy er tilgjengelig via `window.almaBuzzDebug`: `almaBuzzDebug.getState()`, `almaBuzzDebug.playlist.snapshot()`, etc.
...
- **Full syklus + auto-reset**
  - Spill gjennom alle klipp (evt. i testmodus: kjør `almaBuzzDebug.resetPlaylist()` i konsollen og bruk `almaBuzzDebug.playlist.snapshot()` for fremdrift).
  ...
  - Verifiser med `almaBuzzDebug.getState()` for full oversikt over tilstand.
...
- **Debug**
  - `window.almaBuzzDebug.regenByFavorites()` trigger samme logikk for test.
