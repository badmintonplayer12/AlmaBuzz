...
4. **Reset & fail-safe behavior**
   ...
   - Debug helpers (`window.almaBuzzDebug`) can expose `storage.loadFavorites()` output for QA.
...
- Expose `window.almaBuzzDebug.regenByFavorites()` for QA.
...
6. **Debug hook & QA**
   - Add `window.almaBuzzDebug.regenByFavorites()` that calls the same internal routine (skipping fade) for testing.
...
# Implementation Plan: AlmaBuzz PWA
